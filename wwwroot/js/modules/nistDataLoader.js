...
export class NistDataLoader {
    ...
    async getCategorias(funcaoId) {
        if (!funcaoId) return [];
        return this.cache.categorias[funcaoId] ??= await this._fetch(`/Categorias?funcaoId=${funcaoId}`, `categorias da função ${funcaoId}`);
    }
    ...
    async getSubcategorias(categoriaId) {
        ...
        return this.cache.subcategorias[categoriaId] ??= await this._fetch(`/Subcategorias?categoriaId=${categoriaId}`, `subcategorias da categoria ${categoriaId}`);
    }
    ...
    async getInformacoesSubcategoria(subcategoriaId) {
        ...
        try {
            const subcategoria = await this._fetch(`/Subcategorias/${subcategoriaId}`, `subcategoria ${subcategoriaId}`);
            const controles = await this.getControles(subcategoriaId);
            return { subcategoria, controles };
        } catch (error) {
            console.error(`Erro ao carregar informações da subcategoria ${subcategoriaId}:`, error);
    ...
    async getControles(subcategoriaId) {
        ...
        try {
            const controles = await this._fetch(`/Controles?subcategoriaId=${subcategoriaId}`, `controles da subcategoria ${subcategoriaId}`);
    ...
    /** Formata a exibição de uma subcategoria como "FUNCAO.CATEGORIA - SUBCATEGORIA"
     *  @param {Subcategoria} subcategoria
     *  @returns {string}
     */
    formatarSubcategoria(subcategoria) {
        if (!subcategoria) return '';
        return `${subcategoria.funcaoCodigo}.${subcategoria.categoriaCodigo} - ${subcategoria.nome}`;
    }
    ...
    formatarInformacoes(subcategoria, controles) {
        ...
        let texto = '';
        texto += `FUNÇÃO: ${subcategoria.funcaoCodigo} - ${subcategoria.funcaoNome || ''}\n`;
        texto += `CATEGORIA: ${subcategoria.categoriaCodigo} - ${subcategoria.categoriaNome || ''}\n`;
        texto += `SUBCATEGORIA: ${subcategoria.nome || ''}\n`;
        texto += `DESCRIÇÃO: ${subcategoria.descricao || 'Descrição não disponível.'}\n\n`;

        if (controles?.length) {
            texto += 'CONTROLES:\n';
            texto += '─'.repeat(50) + '\n';
            controles.forEach(c => {
                texto += `• ${c.controle || 'Controle'}${c.codigo ? ` (${c.codigo})` : ''}\n`;
            });
        } else {
